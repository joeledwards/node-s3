...
function builder(yargs) {
  yargs
    ...
    .option("types", {
      type: "boolean",
      desc: "include the type of the value associateed with the key in the metric",
      alias: "t",
    });
}
...
async function scan({ aws, options: args }) {
  ...
  function sample(record, depth = 1, path = []) {
    ...
    if (depth > 0) {
      recordType = Array.isArray(record) ? "array" : typeof record;
      ...
      if (isObject) {
        ...
        try {
          ...
          console.warn(
            `Failed to parse JSON from record @ ${pathStr}:`,
            record
          );
        }
      }
    ...
    if (shouldRecurse) {
      ...
      const pathStr = path
        ...
        .reverse()
        .join(".");
      metrics.add(`paths:${pathStr}${types ? `:${recordType}` : ""}`);
    }
  }
